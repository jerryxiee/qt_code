...
FocusScope {
    ...
    ComboBoxButton {
        id: comboBoxButton
        anchors.fill: parent; source: "ico/down.png"
    ...
    Component {
        ...
        Item {
            ...
            Text{
                id:icotext
                color:"white"
                text:typeof(listModel.get(index).attributes) == "undefined"?"":"☞"; font{pixelSize: comboBoxButton.textSize-5; bold: true;}
    ...
    Rectangle{
        ...
        states: [
            State {
                name: "Hide"
                PropertyChanges {
                    target: delegateback; visible:false;height:0;width:0
                }
                StateChangeScript{
                    script: { delegatelistview.focus=false }
                }
            },
            State {
                name: "Show"
                PropertyChanges {
                    target: delegateback;visible:typeof(delegatelistview.model) == "undefined"?false:true; focus: false;height: Math.min(maxHeight, typeof(delegatelistview.model) == "undefined"?0:delegatelistview.model.count*comboBoxButton.height);width:comboBoxButton.width
                }
                StateChangeScript{
                    script: { }
                }
            ...
            StateChangeScript{
                script: {
                    if(currentIndex===beforeDropIndex&&currentIndex2===beforeDropIndex2)
                        return comboBoxSelected(currentIndex,currentIndex2)
